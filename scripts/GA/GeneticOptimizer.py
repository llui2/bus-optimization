...
# Calcular pes de demanda - densitat de població

def compute_demand_weights(NB, pos, res=300):

    """
    Calcula els pesos de demanda per a cada parada de bus amb la densitat gaussiana.

    Parades més properes al centre del camp tindran un valor de pes més alt. Tindran més probabilitat
    de ser seleccionades.

    Args:
        NB (list[int]): Llista d'identificadors de les parades de bus disponibles.
        pos (dict[int, tuple[float, float]]): Diccionari que associa cada node amb la seva posició (x, y).
        res (int, optional): Resolució de la graella per generar el camp de densitat.

    Returns:
        list[float]: Llista de pesos (valors entre 0 i 1 aprox.) per a cada parada de NB.
                    Aquests pesos representen la "importància relativa" de cada parada segons la demanda.
    """

    x_coords = [pos[node][0] for node in NB]
    y_coords = [pos[node][1] for node in NB]
    ...
    for node in NB:
        x, y = pos[node]
        i = int((x - xmin) / (xmax - xmin) * (res - 1))
        j = int((y - ymin) / (ymax - ymin) * (res - 1))
        demand_weights.append(field[j, i])
...
class GeneticOptimizer:
    ...
    def __init__(self, NB, L, n_population, OD, C, seed=None, demand_weights=None):
        self.NB = NB
        self.L = L
    ...
    def _generate_initial_population(self):
        ...
        for _ in range(self.n_population):
            if self.demand_weights:
                # Selecció ponderada segons demanda (demand_weights)
                selected = random.choices(self.NB, weights=self.demand_weights, k=self.L)

                # Eliminar duplicats mantenint l'ordre
                individual = list(dict.fromkeys(selected))

                # Completar si hi ha menys de L nodes de manera aleatòria
                while len(individual) < self.L:
                    candidate = random.choices(self.NB, weights=self.demand_weights)[0]
                    if candidate not in individual:
                        individual.append(candidate)
            else:
                individual = random.sample(self.NB, self.L)
...
if __name__ == "__main__":
    ...
    demand_weights = compute_demand_weights(NB, pos)    # calcula els pesos de demanda segons el camp de densitat per fer la generació ponderada de la població inicial

    optimizer = GeneticOptimizer(NB, L, n_population, data.OD, C, seed, demand_weights)
