...
class GeneticOptimizer:
    ...
    def __init__(self, NB, L, n_population, OD, C, seed=None, debug_log=None):
        self.NB = NB
        self.L = L
    ...
    # 4. --- MUTACIÓ ---
    def mutate(self, individual, prob=0.4):
    ...
    def evolve_one_generation(self, lambda_, n_elite=1, k=3):
        ...
        if self.debug_log:
            with open(self.debug_log, "a") as log:
                log.write(f"  Pares seleccionats per torneig:\n")
                for p in parents:
                    log.write(f"    {p}\n")
        ...
        while len(children) < (self.n_population - n_elite):
            ...
            mutated_child = self.mutate(child, prob=0.2)

            if self.debug_log:
                with open(self.debug_log, "a") as log:
                    log.write(f"  Crossover entre:\n    {p1}\n    {p2}\n")
                    log.write(f"  Fill abans mutació: {child}\n")
                    log.write(f"  Fill després mutació: {mutated_child}\n")
...
if __name__ == "__main__":
    ...
    optimizer = GeneticOptimizer(NB, L, n_population, data.OD, C, seed, debug_log=debug_log_path)
    ...
    for gen in range(n_generations):
        ...
        with open(log_path, "a") as log_file:
            log_file.write(f"Generació {gen+1}:\n")
            log_file.write(f"    Best fitness: {best:.3f}\n")
            log_file.write(f"    Avg fitness: {avg:.3f}\n")
            log_file.write(f"    Millor individu: {optimizer.population[fitnesses.index(best)]}\n")
    ...
    with open(log_path, "a") as log_file:
        ...
        log_file.write(f"Millor línia global: {best_individual}\n")
