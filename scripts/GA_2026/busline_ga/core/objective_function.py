...
class ObjectiveFunction:
    ...
    def estimate_normalization_bounds(
        ...
        """
        Estimate empirical normalization bounds from a set of reference bus lines.

        This helper is suitable for empirical cost bounds and can also provide an
        empirical passenger-service maximum when needed for comparison, although the
        main experiments currently replace that service maximum with a structural
        upper bound derived from the OD matrix.
        """
        raw_evaluations = [
