...
def format_adjusted_line_label(index: int, metrics: Dict[str, float]) -> str:
    label = (
        f"Line {index} | adj_fit={metrics['adjusted_fitness']:.3f} | "
        f"adj_service={metrics['adjusted_passenger_service']:.2f} | "
        f"cost={metrics['cost']:.2f} | shared_edge={metrics['shared_edge_ratio_for_line']:.2f}"
    )
    return label
...
def save_multiline_map(
    ...
    output_png: str,
    title: str,
    adjusted_line_metrics: Optional[Sequence[Dict[str, float]]] = None,
) -> None:
    ...
    for index, line in enumerate(lines, start=1):
        ...
        if adjusted_line_metrics is not None and index - 1 < len(adjusted_line_metrics):
            line_label = format_adjusted_line_label(index, adjusted_line_metrics[index - 1])
        else:
            line_label = format_line_label(index, evaluation)
