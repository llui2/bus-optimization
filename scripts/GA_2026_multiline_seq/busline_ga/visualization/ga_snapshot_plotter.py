...
def save_improvement_snapshot(
    ...
    title = (
        ...
        f"cost_norm={snapshot.cost_norm:.12f} | passenger_service_norm={snapshot.passenger_service_norm:.12f}"
    )
    ax.set_title(title)
...
def save_final_line_with_od(
    ...
    ax.set_title(
        f"{title_prefix}\n"
        f"fit={evaluation.fitness:.12f} | cost={evaluation.cost:.12f} | "
        f"passenger_service={evaluation.passenger_service:.12f}"
    )
