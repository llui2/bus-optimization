...
from busline_ga.visualization.visualize_multiline_ga import save_multiline_map_from_run_result
...
def run_multiline_ga_experiment(
    ...
    print(f"od_min_to_plot: {od_min_to_plot}")
    if save_improvement_snapshots:
    ...
    od_matrix = network.od_matrix
    print("OD shape:", od_matrix.shape)
    print("OD total demand:", od_matrix.sum())
    print("OD max:", od_matrix.max())
    print("OD symmetric:", np.allclose(od_matrix, od_matrix.T))
    print("OD diagonal zero:", np.allclose(np.diag(od_matrix), 0))
    print("OD checksum:", np.round(od_matrix.sum(), 6), np.round(np.linalg.norm(od_matrix), 6))
    ...
    with open(config_path, "w", encoding="utf-8") as config_file:
        ...
        config_file.write(f"od_min_to_plot={od_min_to_plot}\n")
        if save_improvement_snapshots:
    ...
    with open(summary_path, "w", encoding="utf-8") as file:
        ...
        file.write(f"od_min_to_plot: {od_min_to_plot}\n")
        if save_improvement_snapshots:
...
def main() -> None:
    ...
    map_pdf = save_multiline_map_from_run_result(
        run_result=run_result,
        od_min_to_plot=args.od_min_to_plot,
    )
    print("Final multiline map saved to:", map_pdf)
