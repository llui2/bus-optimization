...
def get_project_paths() -> Tuple[str, str, str, str, str]:
    plots_dir = os.path.join(str(RESULTS_DIR), "thesis_evolution_plots")
    ...
    return road_nodes_path, road_edges_path, bus_stops_path, od_matrix_path, plots_dir
...
def main() -> None:
    ...
    best_service_norms: List[float] = []
    best_cost_norms: List[float] = []
    ...
    for generation in range(generations + 1):
        ...
        best_service_norms.append(best_evaluation.passenger_service_norm)
        best_cost_norms.append(best_evaluation.cost_norm)
    ...
    ax.plot(
        ...
        color="#7A0019",
        linewidth=1.6,
        label=r"$C_{\mathrm{norm}}(E_\ell)$",
    )

    configure_axis(ax, "Valor normalitzat")
    ax.set_ylim(0.0, 0.6)
    ...
    out_pdf = os.path.join(plots_dir, "evolution_Pnorm_Cnorm_lambda_05.pdf")
    out_png = os.path.join(plots_dir, "evolution_Pnorm_Cnorm_lambda_05.png")
    ...
    if final_best_evaluation is not None:
        ...
        print("Passenger service norm final:", final_best_evaluation.passenger_service_norm)
        print("Cost norm final:", final_best_evaluation.cost_norm)
